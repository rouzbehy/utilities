...
for eloss in elosses:
    ...
    for cent in jetscape_cents:
        ...
        tmp['othr']  *= Nbin/(factor*(x*dx*inel_Xsec*2*np.pi*2*0.8))
        tmp['dothr'] *= Nbin/(x*dx*inel_Xsec*2*np.pi*2*0.8)
...
for eloss in elosses:
    ...
    for cent in centralities:
        ...
        spec = (jetscape[eloss][cent]['conv']+ jetscape[eloss][cent]['brem']+jetscape[eloss][cent]['othr'])
        dspec =( np.sqrt(jetscape[eloss][cent]['dconv']*jetscape[eloss][cent]['dconv']+\
                        jetscape[eloss][cent]['dbrem']*jetscape[eloss][cent]['dbrem']+
                        jetscape[eloss][cent]['dothr']*jetscape[eloss][cent]['dothr']))
        spec = spec.tolist() 
