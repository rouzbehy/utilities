...
experiments = {0:experiment_pT, 1:experiment_z}
...
for icent, cent in centralities.items():
    axes[0][icent].text(0.05,0.1, f"{cent}$\%$", transform=axes[0][icent].transAxes)
    axes[1][icent].text(0.05,0.1, f"{cent}$\%$", transform=axes[1][icent].transAxes)
axes[0][0].set_xscale('log')
axes[0][0].set_xticks([1,5,10,20,50,100])
axes[0][0].set_xticklabels(['1','5', '10','20','50','100'])
axes[0][1].set_xlabel(r'$p^{h^{\pm}}_{T}$ (GeV)',fontsize=30)
axes[0][0].set_ylabel(r'$R_{D\left(p_T\right)}$', fontsize=30)
axes[0][2].legend(loc='upper right', handles=labels, fontsize=25)

axes[1][0].set_xscale('log')
axes[1][0].set_xticks([0.01,0.04,0.1,0.4,1])
axes[1][0].set_xticklabels(['0.01','0.04','0.1','0.4','1'])
axes[1][1].set_xlabel(r'$z$',fontsize=30)
axes[1][0].set_ylabel(r'$R_{D\left(z\right)}$', fontsize=30)
axes[1][0].text(0.2,0.9,s=r'$100$ GeV $< p^{j}_{T}< 398$ GeV',transform=axes[1][0].transAxes)
axes[1][0].text(0.2,0.8,s=r'$|\eta|<2$',transform=axes[1][0].transAxes)
