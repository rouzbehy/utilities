...
for ir, radius in enumerate(['0.2','0.3','0.4']):
    ...
    for eloss in ['martini', 'cujet']:
        ...
        raa = AA['Ncut']/pp['Ncut']
        draa = raa*np.sqrt(AA['dNcut']*AA['dNcut']/(AA['Ncut']*AA['Ncut']) + pp['dNcut']*pp['dNcut']/(pp['Ncut']*pp['Ncut']) ) 
        #color = ddicts.eloss_colours[f'MATTER+{eloss}'.upper()]
        color = colors[f'MATTER+{eloss}'.upper()]
        axes[ir].plot(AA['pT'], raa, color=color)
        axes[ir].fill_between(AA['pT'], raa+draa,raa-draa,color=color,alpha=0.2)
...
# handles = [Line2D([],[],label='MATTER ($t_{\mathrm{max}}$ = 20 $\mathrm{fm}/c$)', 
#                     color=ddicts.eloss_colours['matter_20'])]
# handles.append(Line2D([],[],label='MATTER ($t_{\mathrm{max}}$ = 200 $\mathrm{fm}/c$)', 
#                     color=ddicts.eloss_colours['matter_200']))
...
handles = [Line2D([],[],color=colors[l], label=l) for l in colors]

handle_exp = [Line2D([],[],marker=ddicts.markers_experiment['cms'],color='black',label='CMS $|\eta|<2$')]


axes[0].legend(handles=handles, bbox_to_anchor=(0.95,0.75), loc='upper left',prop={'size': 18})
axes[1].legend(handles=handle_exp, bbox_to_anchor=(0.95,0.75), loc='upper left',prop={'size': 18})
axes[2].text(0.99,0.7,r'Pb-Pb, $\sqrt{s}=2.76$ ATeV' + "\n" + "$|\eta|<2$" + "\n" + r"$0$-$5\%$", fontsize=20, transform=axes[2].transAxes)
