...
fig, axes = plt.subplots(2,3,gridspec_kw={'top':0.985,'bottom':0.12,
...
for i in jet_radii:
    axes[i].text(0.4, 0.1, r'$R=$'+f"{jet_radii[i]}",transform=axes[i].transAxes)
...
handles = [Line2D([],[]    ,label=r'MATTER, $t_{\mathrm{max}} = 20 \mathrm{fm}/c$', color=ddicts.eloss_colours['matter_20'])]
handles.append(Line2D([],[],label=r'MATTER, $t_{\mathrm{max}} = 200\mathrm{fm}/c$', color=ddicts.eloss_colours['matter_200']))
...
axes[-1].legend(handles=handle_exp, loc='center right',fontsize=18)
axes[-1].text(0.1,0.15,r'Pb-Pb, $\sqrt{s}=2.76$ ATeV' + "\n" + "$|\eta|<2, 0-5\%$", fontsize=20, transform=axes[-1].transAxes)
axes[-1].legend(handles=handles, bbox_to_anchor=(0.1,0.3), loc='lower left',prop={'size': 20})
axes[-1].axis('off')
plt.show()
