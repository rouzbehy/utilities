...
for eloss in elosses:
    for icent, cent in centralities.items():
        ...
        ax.plot(pT, raa, color=ddicts.eloss_colours[eloss])
        ax.fill_between(pT, raa+err, raa-err, color=ddicts.eloss_colours[eloss], alpha=0.3)
        ax.text(0.65, 0.1, f'{ddicts.nice_cent_label[cent]}', transform=ax.transAxes)
...
handles_theory = [Line2D([],[],color=ddicts.eloss_colours[eloss],label=eloss.upper()) for eloss in elosses]
...
ax.text(0.1,0.6,r'$|\eta|<1.0$', transform=ax.transAxes)
ax.text(0.07,0.8,r'Pb-Pb, $\sqrt{s}=2.76$ ATeV', fontsize=30, transform=ax.transAxes)
plt.show()
