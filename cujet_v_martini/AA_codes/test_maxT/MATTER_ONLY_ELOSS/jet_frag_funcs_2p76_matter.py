...
for i in [0,1]:
    ...
    for opt in options:
        ...
        for icent, cent in centralities.items():
            ...
            ax.fill_between(x, ratio+dratio, ratio-dratio, color=color, alpha=0.3)
#labels = []
labels = [Line2D([],[],color=c, label=l) for l,c in colors.items()]
#for eloss in elosses:
#    labels.append(Line2D([],[],label=eloss.upper()+"+MATTER", color=ddicts.eloss_colours[eloss]))#0,marker=ddicts.eloss_marker[eloss]))
#for opt in options:
#    labels.append(Line2D([],[],
#                    label=r'MATTER ($t_{\mathrm{max}}$' + f'={opt}'+ r' $\mathrm{fm}/c$)',
#                    color=ddicts.eloss_colours[f'matter_{opt}']))#,marker=ddicts.eloss_marker[f'matter']))
...
axes[1].legend(loc='lower left', handles=labels)
