...
cents_R_dependence = {0:'00-05', 1:'30-50'}# 1:'05-10',2:'10-30',3:'30-50'}
...
fig, axes = plt.subplots(2,3,sharex='col', sharey='row', figsize=(16,9))

for ir, radius in enumerate(['0.2','0.3','0.4']):
    pp = pp_baseline[radius]
    for icent, cent in cents_R_dependence.items():
        util.plot_expr_data_on_axis(axes[icent][ir],CMS_DATA[cent][radius],marker=ddicts.markers_experiment['cms'])
        for eloss in elosses:
            ...
            axes[icent][ir].plot(AA['pT'], raa, color=color)
            axes[icent][ir].fill_between(AA['pT'], raa+draa,raa-draa,color=color,alpha=0.2)

axes[0][0].set_ylim(top=1.2,bottom=-0.01)
for iax, ax in enumerate(axes[0]):
    ax.text(0.1, 0.1, r'$R=$'+f"{jet_radii[iax]}",transform=ax.transAxes)
...
axes[0][2].legend(handles=handle_exp, loc='upper right',fontsize=25)
axes[1][1].text(0.01,0.95,r'Pb-Pb, $\sqrt{s}=2.76$ ATeV, $|\eta|<2$', fontsize=25, transform=axes[1][1].transAxes)
axes[0][1].legend(handles=handles, loc='upper right',fontsize=25)
