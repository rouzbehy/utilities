...
if do_incnlo:
    ...
    for cent in other_photons:
        yy  = kfactor* tmp_y*multiplicities[cent]
        dyy = np.sqrt(kfactor)* tmp_dy*multiplicities[cent] #np.array([average_err * yval for yval in yy])
...
fig3, axes3 = plt.subplots(2, 2, figsize=(16,9), sharex=True, sharey=True)
## plot channel ratios
for icent, cent in enumerate(['00-10','10-20']):
    row = axes3[icent]
    row[0].text(0.05,0.8, cent+r'$\%$', transform=row[0].transAxes)
    spec = ratios[cent]
...
axes3[1][1].legend(loc='center right', handles=handles, fontsize=20)
axes3[0][1].legend(loc='center right', handles=jf_handles, fontsize=20)
axes3[0][0].text(0.5,0.6, r'Au-Au @ $\sqrt{s}=200$ AGeV', transform=axes3[0][0].transAxes, fontsize=20)
axes3[0][0].text(0.5,0.5, r'$|\eta|<0.35$', transform=axes3[0][0].transAxes, fontsize=20)
plt.show()
