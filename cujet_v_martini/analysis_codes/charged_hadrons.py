...
def get_hadrons_single_file(fname, hist, ch_had_eta_cut):
    ...
    for line in open(fname, 'r'):
        ...
        phi = float(phi)
        eta = float(eta)
        if pid not in PDG.PDG_Pool : continue
        if PDG.PDG_Charge[pid] == 0 : continue
...
def build_charged_spec(data_loc, pTHatbins, num_runs, pTbins, ch_had_eta_cut):
    ...
    for ibin in range(num_pTHat_bins):
        ...
        tmp_hist = collect_hadrons_pTHatbin(data_loc, pTmin, pTmax, num_runs, pTbins,ch_had_eta_cut)
...
if __name__=='__main__':
    ...
    hist = build_charged_spec(location, pThats, num_runs, prop['spectrum bins'], prop['eta cut'])
