...
class Histogram:
    def __init__(self, xvals: List[float]):
        self.bins = array(xvals)
        self.nbins = len(xvals) - 1
        self.hist = array([0.0 for x in range(self.nbins)])
        self.err = array([0.0 for x in range(self.nbins)])
        self.nentries = 0

    def fill(self, xval: float, weight: float = 1):
    ...
    def multiply_by(self, number: float):
    ...
    def set_content(self, yvals: List[float], dyvals: List[float]):
        self.hist = yvals
...
class Jet:
    ...
    def calc_z(self, hadron):
        ...
        pJet_dot_pTrk = self.pT * hadron.pT * cos(
            delta_phi
        ) + self.en * hadron.en * tanh(self.eta) * tanh(hadron.eta)
