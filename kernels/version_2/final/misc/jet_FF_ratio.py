...
def get_martini_results(rateset, cent):
    fname_pt = f"../../../calcs/final_runs/rset_{rateset}/2p76/{cent}/jet_FF_pT.csv"
    fname_z = f"../../../calcs/final_runs/rset_{rateset}/2p76/{cent}/jet_FF_z.csv"
...
## Read in the pp data:
pp_DPT = pd.read_csv("../../../calcs/pp_2p76/jet_FF_pT.csv", comment="#")
pp_DZ = pd.read_csv("../../../calcs/pp_2p76/jet_FF_z.csv", comment="#")
...
njets_pp, nevts_pp = 0, 0
with open("../../../calcs/pp_2p76/jet_FF_pT.csv", "r") as f:
    line = f.readline()
    line = line.split(" ")
    njets_pp = float(line[1])
    nevts_pp = float(line[-1])
...
for rset in rate_names:
    ...
    pT1, dpT1, tmpy1, dtmpy1, z1, dz1, tmpyz1, dtmpyz1 = get_martini_results(
        rset, "0_5"
    )
    pT2, dpT2, tmpy2, dtmpy2, z2, dz2, tmpyz2, dtmpyz2 = get_martini_results(
        rset, "5_10"
    )
...
fig, axes = plt.subplots(
    ...
    figsize=(16, 9),
    sharex=False,
    #  gridspec_kw={'top':0.985,
    #         'bottom':0.15, 'left':0.08,
    #         'right':0.95, 'hspace':0.05,
    #         'wspace':0.3},
    sharey=True,
)
...
for rate_set in rate_names:
    ...
    dratio = np.array(
        [
            r * np.sqrt(erra * erra / (va * va) + errp * errp / (vp * vp))
...
axes[1].legend(loc="upper right", handles=labels, handletextpad=0.05)
axes[0].set_xscale("log")
axes[0].set_ylabel(r"$R_{D(p_T)}$")
axes[1].set_ylabel(r"$R_{D(z)}$")
axes[0].set_ylim(bottom=0.79, top=1.72)
...
# plt.savefig("../../plots/compare_coll_kernels/jet_FF_ratio.png", dpi=200)
plt.show()
